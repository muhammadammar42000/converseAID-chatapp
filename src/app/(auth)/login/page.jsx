    ...
    signInWithEmailAndPassword(auth, email, password)
      ...
      .catch((error) => {
        ...
        toast({
          position: 'top-right',
          status: 'error',
          title: errorCode??'Invalid Credintial',
          duration: 9000,
          isClosable: true
        })
        setLoading(false)
    ...
    signInWithPopup(auth,provider)
      .then(async (result) => {
        ...
        if (!querySnapshot.empty) {
          ...
          const userDataOne = querySnapshot.docs[0].data()
          // Combine user auth data with Firestore data
          const userData = { uid: userDataOne?.userId, email: userDataOne.email, ...userDataOne }
          setUser(userData) // Assuming you have a setUser function to set user state
          toast({
            position: 'top-right',
            status: 'success',
        ...
        if (error.code === 'auth/account-exists-with-different-credential') {
          ...
          if (methods.length > 0) {
            alert(`This email is already associated with ${methods[0]}. Please use that method to log in.`);
          } else {
            alert('This email is already associated with another account. Please try a different login method.');
          }
        } else {
          // Handle other errors
